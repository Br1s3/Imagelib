...
void draw_cercle(uint8_t ***pixels, short width, short height, int x, int y, int radius, const uint32_t fd, int nb_color);
...
void draw_cercle(uint8_t ***pixels, short width, short height, int x, int y, int radius, const uint32_t fd, int nb_color)
{
   ...
   for (i = wind.A.y; i < wind.B.y; i++) {
	  ...
	  if ((i-y)*(i-y) + (j-x)*(j-x) <= radius*radius) {
		  ...
		  pixels[i-wind.A.y][j-wind.A.x][k] |= fd>>(8*(nb_color-k-1));
	      }
	  }
      }
   }
